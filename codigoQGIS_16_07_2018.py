...
while i< count:
    ...
    images = [item.get('id') for item in sentinelAOI.getInfo().get('features')]
...
for elements in range(0,len(featuresNull)):
    ...
    for noNullElements in range(0, len(featuresNotNull)):
        print("QUE VALOR? ",noNullElements, featuresNotNull[noNullElements][1])
...
chunks = [datosDistancias[x:x+len(featuresNull)] for x in xrange(0, len(datosDistancias), len(featuresNull))]
numDim= len(featuresNull)
datafinal=[]
for c in range(0, numDim):
    print("C ", c)
    datafinal.append(zip(*chunks)[c])
dist = pd.DataFrame(data=datafinal, columns=nombreColumnas)
