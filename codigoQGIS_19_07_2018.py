...
dfni= dfni.sort_index(axis='index',kind='heapsort')
...
for elements in range(0,len(featuresNull)):
    print("QUE ELEMENTO NULL? ",  featuresNull[elements][3])
    clases.append(featuresNull[elements][3])
    nombreColumnas=[]
    for noNullElements in range(len(featuresNull),len(dfni.index)):
        print("QUE VALOR? ",noNullElements,dfni.iloc[[noNullElements]].index)
        nombreColumnas.append(dfni.iloc[[noNullElements]].index[0])
        #indice= nullElements+len(featuresNotNull)
        #prueba = dfni.loc[featuresNotNull[noNullElements][1]]-dfni.iloc[[elements]]
        prueba1 = dfni.iloc[[noNullElements]].values
        prueba2 = dfni.iloc[[elements]].values
...
distR=dist.iloc[::-1]
fp5.write(distR.to_string())
fp5.close()
predictedClases= distR.iloc[:,-1]
